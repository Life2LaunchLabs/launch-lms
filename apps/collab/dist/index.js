    ...
    const timer = setTimeout(async () => {
        ...
        try {
            ...
            const response = await fetchWithTimeout(url, {
                ...
                body: stateCopy.buffer.slice(stateCopy.byteOffset, stateCopy.byteOffset + stateCopy.byteLength),
            });
            if (!response.ok) {
...
const server = new Server({
    ...
    async onAuthenticate({ token, documentName }) {
        ...
        let response;
        try {
            response = await fetchWithTimeout(`${API_URL}/api/v1/boards/${boardUuid}/membership`, {
                headers: {
            ...
            console.error(`[collab] Membership check failed for ${boardUuid}:`, err);
