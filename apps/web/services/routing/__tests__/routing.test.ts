...
test('route manifest builds key dashboard and owner routes', () => {
  ...
  assert.equal(routePaths.org.dash.users.groupProgram(2, 'assignment_1'), '/admin/users/groups/2/programs/assignment_1')
...
test('route manifest builds auth, account, and public org paths used by navigation surfaces', () => {
  ...
  assert.equal(routePaths.org.programs(), '/badges/programs')
  assert.equal(routePaths.org.program('participant-1'), '/badges/programs/participant-1')
