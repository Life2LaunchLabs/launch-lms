...
export default function OrganizationDetail({
  ...
  if (!org) {
    return (
      <div className={`flex flex-col items-center justify-center py-20 ${lightTheme ? 'text-gray-500' : 'text-white/40'}`}>
  ...
  return (
    <div className={`p-8 ${lightTheme ? 'platform-detail-light' : ''}`}>
      <div className="mb-6">
        <Link
          href={backHref}
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              {(org.pending_request_count ?? 0) > 0 && (
                <span className="text-[10px] font-medium uppercase tracking-wider px-2 py-0.5 rounded bg-amber-400/10 text-amber-400">
                  {org.pending_request_count} pending request{org.pending_request_count === 1 ? '' : 's'}
                </span>
              )}
            </div>
          </div>
      ...
      {lightTheme && (
        <style jsx global>{`
          .platform-detail-light [class~="text-white"] { color: rgb(17 24 39) !important; }
          .platform-detail-light [class~="text-white/80"] { color: rgb(31 41 55) !important; }
          .platform-detail-light [class~="text-white/70"] { color: rgb(55 65 81) !important; }
          .platform-detail-light [class~="text-white/60"] { color: rgb(75 85 99) !important; }
          .platform-detail-light [class~="text-white/50"] { color: rgb(107 114 128) !important; }
          .platform-detail-light [class~="text-white/40"] { color: rgb(107 114 128) !important; }
          .platform-detail-light [class~="text-white/30"] { color: rgb(156 163 175) !important; }
          .platform-detail-light [class~="text-white/25"] { color: rgb(156 163 175) !important; }
          .platform-detail-light [class~="text-white/20"] { color: rgb(209 213 219) !important; }
          .platform-detail-light [class~="bg-white/[0.03]"] { background-color: rgb(255 255 255) !important; }
          .platform-detail-light [class~="bg-white/[0.05]"] { background-color: rgb(249 250 251) !important; }
          .platform-detail-light [class~="bg-white/[0.06]"] { background-color: rgb(243 244 246) !important; }
          .platform-detail-light [class~="bg-white/[0.08]"] { background-color: rgb(243 244 246) !important; }
          .platform-detail-light [class~="border-white/[0.05]"] { border-color: rgb(243 244 246) !important; }
          .platform-detail-light [class~="border-white/[0.08]"] { border-color: rgb(229 231 235) !important; }
          .platform-detail-light [class~="border-white/[0.15]"] { border-color: rgb(209 213 219) !important; }
          .platform-detail-light [class~="border-white"] { border-color: rgb(17 24 39) !important; }
          .platform-detail-light [class~="hover:bg-white/[0.03]"]:hover { background-color: rgb(249 250 251) !important; }
          .platform-detail-light [class~="hover:bg-white/[0.05]"]:hover { background-color: rgb(243 244 246) !important; }
          .platform-detail-light [class~="hover:bg-white/[0.08]"]:hover { background-color: rgb(243 244 246) !important; }
          .platform-detail-light [class~="hover:text-white"]:hover { color: rgb(17 24 39) !important; }
          .platform-detail-light [class~="hover:text-white/60"]:hover { color: rgb(75 85 99) !important; }
          .platform-detail-light [class~="hover:border-white/[0.15]"]:hover { border-color: rgb(209 213 219) !important; }
          .platform-detail-light [class~="hover:border-white/20"]:hover { border-color: rgb(209 213 219) !important; }
        `}</style>
      )}
    </div>
  )
...
function AnalyticsTab({ orgId, accessToken }: { orgId: string; accessToken: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-white/30 text-sm">No analytics available in this environment.</div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        {saved && <p className="text-sm text-emerald-400 mt-2">Plan updated successfully</p>}
        ...
        <div className="bg-white/[0.03] border border-white/[0.08] rounded-xl divide-y divide-white/[0.06]">
          {Object.entries(features).map(([key, val]: [string, any]) => (
            <div key={key} className="flex items-center justify-between px-5 py-3">
              <div className="flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full ${val?.enabled !== false ? 'bg-emerald-400' : 'bg-white/20'}`} />
...
function SettingsTab({
  ...
  const handleSave = async () => {
    ...
    setError('')
    try {
      const res = await fetch(`${getAPIUrl()}superadmin/organizations/${orgId}/settings`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${accessToken}`,
    ...
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <div className="space-y-4">
          ...
          <Field label="Slug" value={form.slug} onChange={(v) => setForm({ ...form, slug: v })} mono hint="Used in URLs. Changing this will break existing links." />
...
function RequestsTab({ orgId, accessToken }: { orgId: string; accessToken: string }) {
  const { data: requests, isLoading, mutate: mutateRequests } = useSWR<any[]>(
  ...
  async function handleUpdateStatus(requestUuid: string, status: 'approved' | 'denied') {
    ...
    try {
      const res = await fetch(`${getAPIUrl()}superadmin/plan-requests/${requestUuid}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${accessToken}`,
      ...
      await mutate(`${getAPIUrl()}superadmin/organizations/${orgId}`)
    } finally {
      setUpdating(null)
    }
  }
          ...
          <tbody>
            {requests.map((req: any) => (
              <tr key={req.request_uuid} className="border-b border-white/[0.05] last:border-0">
                ...
                <td className="px-4 py-3 text-white/40 text-xs max-w-[200px] truncate">{req.message || '—'}</td>
                <td className="px-4 py-3">
