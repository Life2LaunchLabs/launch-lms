...
export default function ProgramAssignmentWizard({ orgslug, programUuid, initialGroupId }: { orgslug: string; programUuid: string; initialGroupId?: string }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      router.push(getUriWithOrg(orgslug, initialGroupId ? `/admin/users/groups/${initialGroupId}/programs` : routePaths.org.dash.programPage(programUuid, 'assignments')))
  ...
  return <main className="min-h-full bg-[#f8f8f8] px-6 py-7"><div className="mx-auto max-w-6xl">
    <Link href={getUriWithOrg(orgslug, initialGroupId ? `/admin/users/groups/${initialGroupId}/programs` : routePaths.org.dash.programPage(programUuid, 'assignments'))} className="inline-flex items-center gap-1.5 text-sm font-bold text-muted-foreground hover:text-foreground"><ArrowLeft size={14} />{initialGroupId ? 'Back to group programs' : 'Back to assignments'}</Link>
