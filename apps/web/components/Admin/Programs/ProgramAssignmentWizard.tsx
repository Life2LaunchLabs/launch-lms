...
export default function ProgramAssignmentWizard({ orgslug, programUuid, initialGroupId }: { orgslug: string; programUuid: string; initialGroupId?: string }) {
  ...
  const { data: program, isLoading } = useSWR(orgId && token ? `${getAPIUrl()}planning/templates/${programUuid}?org_id=${orgId}` : null, (url) => swrFetcher(url, token))
  ...
  const [step, setStep] = React.useState(initialGroupId ? 1 : 0)
  ...
  const next = async () => {
    if (step === 0 && creatingGroup) {
      ...
      } catch (error: any) { toast.error(error?.message || 'Could not create the group.'); return } finally { setSaving(false) }
    }
    if (step === 0 && !groupId && !creatingGroup) return toast.error('Choose or create a group.')
    if (step === 1 && (!staffIds.length || !ownerId)) return toast.error('Assign staff and choose the accountable plan owner.')
    if (step === 2) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const assignment = await programsApi.assign(orgId, programUuid, {
        usergroup_id: Number(groupId), staff_user_ids: staffIds, owner_user_id: Number(ownerId),
    ...
    } catch (error: any) { toast.error(error?.message || 'Could not assign the plan.') } finally { setSaving(false) }
  }

  return <main className="min-h-full bg-[#f8f8f8] px-6 py-7"><div className="mx-auto max-w-6xl">
    <Link href={getUriWithOrg(orgslug, routePaths.org.dash.planAssignments())} className="inline-flex items-center gap-1.5 text-sm font-bold text-muted-foreground hover:text-foreground"><ArrowLeft size={14} />Back to assignments</Link>
    <div className="mt-5 grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)]"><aside><div className="rounded-xl border border-border bg-card p-4 shadow-xs"><div className="flex items-center gap-3 border-b border-border pb-4"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-50 text-blue-600"><Layers3 size={17} /></span><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Assign plan</p><p className="truncate text-sm font-black">{program.name}</p></div></div><ol className="mt-4 space-y-1">{steps.map((label, index) => <li key={label}><button onClick={() => index < step && setStep(index)} disabled={index > step} className={cn('flex w-full items-center gap-3 rounded-lg px-2 py-2.5 text-left text-xs font-bold', index === step ? 'bg-foreground text-background' : index < step ? 'text-foreground hover:bg-muted' : 'text-muted-foreground')}><span className={cn('flex h-5 w-5 items-center justify-center rounded-full border text-[10px]', index < step && 'border-green-600 bg-green-600 text-white')}>{index < step ? <Check size={11} /> : index + 1}</span>{label}</button></li>)}</ol></div></aside>
      <section className="rounded-xl border border-border bg-card shadow-xs"><div className="border-b border-border px-6 py-5"><p className="text-xs font-black uppercase tracking-widest text-blue-600">Step {step + 1} of 4</p><h1 className="mt-1 text-2xl font-black">{step === 0 ? 'Choose a group' : step === 1 ? 'Assign staff' : step === 2 ? 'Set the schedule' : 'Prepare the invitation'}</h1><p className="mt-1 text-sm text-muted-foreground">{step === 0 ? 'Use an existing group or create one here.' : step === 1 ? 'Choose the staff responsible for this plan.' : step === 2 ? 'Set exact phase dates and customize objective rules.' : 'Choose when invitations go out and add a welcome message.'}</p></div>
        <div className="min-h-[440px] p-6">
          {step === 0 && <div className="grid gap-3 sm:grid-cols-2">{(groups || []).map((group: any) => <button key={group.id} onClick={() => { setGroupId(String(group.id)); setCreatingGroup(false) }} className={cn('flex items-center gap-3 rounded-xl border p-4 text-left', groupId === String(group.id) ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-100' : 'border-border hover:bg-muted/40')}><Users size={19} className="text-blue-600" /><div><p className="text-sm font-black">{group.name}</p><p className="mt-0.5 line-clamp-1 text-xs text-muted-foreground">{group.description || 'Group'}</p></div></button>)}<button onClick={() => { setCreatingGroup(true); setGroupId('') }} className={cn('flex items-center gap-3 rounded-xl border border-dashed p-4 text-left', creatingGroup ? 'border-blue-500 bg-blue-50' : 'border-border hover:bg-muted/40')}><Plus size={19} /><span className="text-sm font-black">Create a group</span></button>{creatingGroup && <div className="space-y-3 rounded-xl border border-blue-200 bg-blue-50/40 p-4 sm:col-span-2"><Field label="Group name"><input autoFocus value={groupName} onChange={(e) => setGroupName(e.target.value)} className="h-10 w-full rounded-lg border border-border bg-card px-3 text-sm" /></Field><Field label="Description (optional)"><textarea value={groupDescription} onChange={(e) => setGroupDescription(e.target.value)} rows={2} className="w-full rounded-lg border border-border bg-card p-3 text-sm" /></Field></div>}</div>}
          {step === 1 && <div className="space-y-6"><StaffStep members={members} staffIds={staffIds} setStaffIds={(updater) => { setStaffIds((current) => { const next = typeof updater === 'function' ? updater(current) : updater; if (!ownerId && next.length) setOwnerId(String(next[0])); if (ownerId && !next.includes(Number(ownerId))) setOwnerId(next.length ? String(next[0]) : ''); return next }); }} /><Field label="Accountable plan owner"><select value={ownerId} onChange={(event) => setOwnerId(event.target.value)} className="h-10 w-full max-w-md rounded-lg border border-border bg-card px-3 text-sm"><option value="">Choose an owner</option>{members.filter((membership: any) => staffIds.includes(Number((membership.user || membership).id))).map((membership: any) => { const person = membership.user || membership; return <option key={person.id} value={person.id}>{person.first_name || person.username} {person.last_name || ''}</option> })}</select><p className="mt-1 text-xs font-normal text-muted-foreground">This person is accountable for each learner's independent plan and may transfer ownership later.</p></Field></div>}
...
function StaffStep({ members, staffIds, setStaffIds }: { members: any[]; staffIds: number[]; setStaffIds: React.Dispatch<React.SetStateAction<number[]>> }) {
  return <div>
    ...
    <div className="grid gap-2 sm:grid-cols-2">
      ...
      {!members.length && <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground sm:col-span-2">No organization staff are available. Give an organization member a staff role before assigning this plan.</div>}
    </div>
  </div>
}
...
function ScheduleStep({ program, initiateDate, setInitiateDate, phaseDates, updatePhase, objectiveRules, updateObjective }: any) {
  return <div className="schedule-compact space-y-5">
    ...
    <div className="flex flex-wrap items-end gap-4 rounded-xl border border-border bg-muted/30 p-4">
      ...
      <p className="max-w-xl pb-1 text-xs leading-5 text-muted-foreground">Invitations become visible on this date. Accepted learners can view the plan before objectives open.</p>
    </div>
