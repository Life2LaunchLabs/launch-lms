...
export default function ProgramObjectiveEditorRow({ objective, orgId, token, program, dragHandleProps, dragging }: any) {
  ...
  const [fields, setFields] = React.useState<EvidenceField[]>(objective.custom_fields || [])
  const [allowConfirmation, setAllowConfirmation] = React.useState(Boolean(objective.allow_learner_confirmation))
  ...
  React.useEffect(() => {
    ...
    setFields(objective.custom_fields || [])
    setAllowConfirmation(Boolean(objective.allow_learner_confirmation))
  ...
  const save = async () => {
    ...
    try {
      await programsApi.updateObjective(orgId, program.program_uuid, objective.objective_uuid, {
        title: title.trim(), description, custom_fields: fields,
        allow_learner_confirmation: allowConfirmation,
