...
import GroupPlanWorkspace from '@components/Plans/GroupPlanWorkspace'
...
const assignmentTabs = [
  ...
  { id: 'progress' as const, label: 'Objectives', icon: Gauge },
  ...
  return (
    <main className="min-h-full w-full bg-[#f8f8f8]">
      <div className="relative z-10 bg-[#fcfbfc] px-10 tracking-tight nice-shadow">
        <div className="pb-4 pt-6"><Breadcrumbs items={[{ label: 'Plans', href: getUriWithOrg(orgslug, routePaths.org.dash.programs()) }, { label: 'Assignments', href: getUriWithOrg(orgslug, routePaths.org.dash.planAssignments()) }, { label: data.program?.name || 'Plan assignment' }]} /></div>
        <div className="flex flex-col gap-4 pb-5 md:flex-row md:items-center md:justify-between">
          <div><div className="flex flex-wrap items-center gap-3"><h1 className="text-2xl font-black text-gray-950">{data.program?.name}</h1><span className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2.5 py-1 text-xs font-bold text-gray-500"><Users size={13} />{data.learners.length} learner{data.learners.length === 1 ? '' : 's'}</span></div><p className="mt-1 text-sm text-gray-500">Assigned to {data.cohort?.name || [data.assignment?.user?.first_name, data.assignment?.user?.last_name].filter(Boolean).join(' ') || data.assignment?.user?.username || data.assignment?.subject_email || 'an individual learner'}</p></div>
          {data.programs.length > 1 ? <div className="inline-flex max-w-full overflow-x-auto rounded-xl border border-gray-200 bg-white p-1">{data.programs.map((program: any) => <button key={program.assignment_uuid} onClick={() => chooseProgram(program.assignment_uuid)} className={cn('min-w-36 rounded-lg px-3 py-2 text-left transition', program.assignment_uuid === assignmentUuid ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50')}><span className="block truncate text-xs font-black">{program.program_name}</span><span className={cn('block text-[10px] font-semibold', program.assignment_uuid === assignmentUuid ? 'text-blue-100' : 'text-gray-400')}>{program.progress_percent}% complete</span></button>)}</div> : null}
        </div>
      ...
      <div className="mx-auto max-w-[1500px] px-6 py-6">
        {activeSubpage === 'progress' ? <GroupPlanWorkspace orgslug={orgslug} assignmentUuid={assignmentUuid} embedded /> : null}
        ...
        {data.use_legacy_progress_matrix === true ? <>
