...
export default function CandidateToolbar({ accessToken, orgId, activePanel, onOpen, onConfiguration, onUnreadAnnouncements }: {
  ...
  const actionClass = (panel: CandidatePanel) => `relative flex h-9 min-w-0 items-center gap-1.5 rounded-lg px-2.5 text-xs font-semibold transition ${activePanel === panel ? 'bg-amber-300 text-amber-950' : 'hover:bg-amber-200/60'}`
  return <div className="flex min-h-12 w-full min-w-0 max-w-full items-center justify-between gap-2 overflow-hidden border-b border-amber-300 bg-amber-50 px-3 text-amber-950 dark:border-amber-300/25 dark:bg-[#211c10] dark:text-amber-100 sm:px-5">
    ...
    <div className="flex min-w-0 shrink-0 items-center gap-1">
      <button type="button" aria-expanded={activePanel === 'announcements'} onClick={() => onOpen('announcements')} className={actionClass('announcements')} aria-label={unread.announcements ? `Announcements, ${unread.announcements} unread` : 'Announcements'}><Megaphone /> <span className="hidden sm:inline">Announcements</span>{unread.announcements ? <UnreadCount value={unread.announcements} /> : null}</button>
      ...
      <button type="button" aria-expanded={activePanel === 'feedback'} onClick={() => onOpen('feedback')} className={actionClass('feedback')} aria-label={unread.feedback ? `Feedback, ${unread.feedback} unread` : 'Feedback'}><ChatCircleDots /> <span className="hidden sm:inline">Feedback</span>{unread.feedback ? <UnreadCount value={unread.feedback} /> : null}</button>
    </div>
  </div>
}
