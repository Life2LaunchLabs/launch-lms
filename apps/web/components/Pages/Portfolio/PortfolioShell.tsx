...
type Shell = { portfolio: Record<string, any>; work: Work[]; journey: JourneyEntry[]; views: Array<{ key: string; visible: boolean; itemCount: number }>; readiness: { canPublish: boolean; blockers: string[] }; nextAction?: { label: string; href: string } | null }

function tabs(orgslug: string, username: string | undefined, owner: boolean, shell: Shell) {
  const base = owner ? '/portfolio' : `/user/${username}`
  return [{ label: 'Overview', view: 'overview', href: base, visible: true }, { label: 'Journey', view: 'journey', href: `${base}/journey`, visible: owner || shell.views.some((view) => view.key === 'journey' && view.visible) }, { label: 'Work', view: 'work', href: `${base}/work`, visible: owner || shell.views.some((view) => view.key === 'work' && view.visible) }]
    ...
    <motion.header layout className={`sticky top-0 z-[var(--z-sticky-header)] border-b border-border/70 bg-background/95 backdrop-blur-xl ${compact ? 'shadow-[0_1px_0_hsl(var(--border)/.25)]' : ''}`} transition={reduceMotion ? { duration: 0 } : { type: 'spring', stiffness: 320, damping: 34 }}>
      <motion.div layout className={`mx-auto max-w-5xl px-5 sm:px-8 ${compact ? 'py-3' : 'py-8 sm:py-12'}`}>
        <motion.div layout className={`${compact ? 'flex items-center justify-between gap-4' : 'flex flex-col items-center gap-5 text-center sm:flex-row sm:items-center sm:text-left'}`}>
          ...
          {owner && !preview && compact && activeView === 'journey' && <Button asChild size="sm"><Link href={getUriWithOrg(orgslug, '/portfolio/journey/new')}><Plus className="mr-1.5 h-4 w-4" />Add chapter</Link></Button>}
        </motion.div>
      </motion.div>
      <nav className="mx-auto flex max-w-5xl gap-7 overflow-x-auto px-5 sm:px-8" aria-label="Portfolio views">{nav.map((tab) => { const view = tab.view as 'overview' | 'journey' | 'work'; const selected = activeView === view; return <button type="button" key={tab.label} onClick={() => changeView(view, tab.href)} className={`relative py-3 text-sm font-semibold transition-colors ${selected ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>{tab.label}{selected && <motion.span layoutId="portfolio-active-tab" className="absolute inset-x-0 bottom-0 h-0.5 rounded-full bg-foreground" transition={{ type: 'spring', stiffness: 500, damping: 38 }} />}</button>})}</nav>
    </motion.header>
