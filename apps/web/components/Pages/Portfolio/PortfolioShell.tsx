...
function Overview({ shell, owner, orgslug, username, importLegacy, busy }: any) {
  ...
  return <div className="space-y-14">
    {shell.nextAction && owner && <section className="flex flex-col gap-5 rounded-2xl border border-border bg-muted/35 p-6 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm text-muted-foreground">Your next portfolio step{shell.nextAction.estimatedMinutes ? ` · about ${shell.nextAction.estimatedMinutes} min` : ''}</p><h2 className="mt-1 text-xl font-bold">{shell.nextAction.label}</h2>{shell.nextAction.supportingText && <p className="mt-2 max-w-xl text-sm text-muted-foreground">{shell.nextAction.supportingText}</p>}{shell.nextAction.progress && <div className="mt-4 h-1.5 w-52 overflow-hidden rounded-full bg-border"><div className="h-full rounded-full bg-foreground" style={{ width: `${Math.round((shell.nextAction.progress.completed / Math.max(1, shell.nextAction.progress.total)) * 100)}%` }}/></div>}</div><Button asChild><Link href={getUriWithOrg(orgslug, shell.nextAction.href)}><Sparkles className="mr-2 h-4 w-4" />{shell.nextAction.progress?.completed ? 'Continue' : 'Get started'}</Link></Button></section>}
