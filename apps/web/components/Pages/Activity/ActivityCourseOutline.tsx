...
export default function ActivityCourseOutline({
  ...
  return (
    <div className={`flex min-h-0 flex-col ${isSheet ? 'h-full' : 'max-h-[600px] overflow-hidden rounded-lg bg-card drop-shadow-xs'}`}>
      <div>
        {headerMode === 'back' ? (
          <div className={`${isSheet ? 'px-3 pb-2 pt-3' : 'px-3 pb-2 pt-3'}`}>
            <Link
              ...
              className="flex min-w-0 items-center gap-2 rounded-lg px-2 py-2 text-foreground transition-colors hover:bg-muted"
            >
          ...
          <div className={`flex items-center gap-2 ${isSheet ? 'px-4 pb-2 pt-4' : 'px-5 pb-2 pt-4'}`}>
            ...
            <p className="text-xs text-muted-foreground">{course.chapters?.length || 0} chapters</p>
          </div>
        )}
            ...
            return (
              <div key={chapterKey} className="overflow-hidden rounded-2xl">
                <button
                  ...
                  className={`flex w-full items-center gap-3 px-2 py-3 text-left transition-colors hover:bg-muted ${isExpanded ? 'bg-muted' : ''}`}
                >
                      ...
                      return (
                        <Link
                          ...
                          className={`flex items-center gap-3 pl-8 pr-4 py-3 transition-colors ${
                            isHighlighted ? 'bg-emerald-50/80' : 'hover:bg-muted'
                          }`}
                        >
                          {getActivityStatusIcon(activity, isHighlighted)}
                          <p className={`min-w-0 flex-1 truncate text-sm font-medium ${isHighlighted ? 'text-emerald-950' : 'text-foreground'}`}>
                            {activity.name}
                          </p>
                        </Link>
                      )
