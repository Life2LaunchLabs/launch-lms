                ...
                <>
                    ...
                    <div className="ml-10 mr-10 mx-auto bg-white rounded-xl shadow-xs px-4 py-4 anit ">
                        <div className="flex flex-col bg-gray-50 -space-y-1  px-5 py-3 rounded-md mb-3 ">
                            <h1 className="font-bold text-xl text-gray-800">{t('dashboard.users.invite_members.title')}</h1>
                        ...
                        <div className="flex space-x-2 mx-auto my-5 ml-2 items-center space-x-4 justify-between">
                            ...
                            <div className='flex space-x-2 items-center'>
                                <p className='flex items-center'>{t('dashboard.users.invite_members.invite_code_label')} </p>
                                <select
                                ...
                                <ToolTip content={t('dashboard.users.invite_members.invite_code_tooltip')} sideOffset={8} side="right"><Info className='text-gray-400' size={14} /></ToolTip>
                            </div>
                            ...
                            <table className="table-auto w-full text-left whitespace-nowrap rounded-md overflow-hidden">
                                <thead className="bg-gray-100 text-gray-500 rounded-xl uppercase">
                                    <tr className="font-bolder text-sm">
                                        <th className="py-3 px-4">{t('dashboard.users.invite_members.invited_users.table.email')}</th>
                                        <th className="py-3 px-4">{t('dashboard.users.invite_members.invited_users.table.signup_status')}</th>
                                        <th className="py-3 px-4">{t('dashboard.users.invite_members.invited_users.table.email_sent')}</th>
                                    </tr>
                                </thead>
                                <>
                                    <tbody className="mt-5 bg-white rounded-md">
                                        {invited_users?.map((invited_user: any) => (
                                            ...
                                            >
                                                ...
                                                <td className="py-3 px-4">{invited_user.pending ? <div className='bg-orange-400 text-orange-100 w-fit px-2 py1 rounded-md'>{t('dashboard.users.invite_members.invited_users.status.pending')}</div> : <div className='bg-green-400 text-green-100 w-fit px-2 py1 rounded-md'>{t('dashboard.users.invite_members.invited_users.status.signed')}</div>}</td>
                                                <td className="py-3 px-4">{invited_user.email_sent ? <div className='bg-green-600 text-green-100 w-fit px-2 py1 rounded-md'>{t('dashboard.users.invite_members.invited_users.email_status.sent')}</div> : <div className='bg-red-400 text-red-100 w-fit px-2 py1 rounded-md'>{t('dashboard.users.invite_members.invited_users.email_status.no')}</div>}</td>
