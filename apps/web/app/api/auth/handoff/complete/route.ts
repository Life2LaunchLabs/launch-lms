...
export async function POST(request: NextRequest) {
  ...
  if (getConfig('NEXT_PUBLIC_LAUNCHLMS_COOKIE_SCOPE', 'shared-domain') !== 'host-only' ||
      !isManagedHost(publicUrl.host, domain) ||
  ...
  try {
    const result = await fetch(`${BACKEND_URL}/api/v1/auth/handoff/redeem`, {
      ...
      body: JSON.stringify({ ticket, target_host: publicUrl.host, state, return_path: returnPath }),
    ...
    const response = NextResponse.redirect(new URL(returnPath, publicUrl), { status: 303 })
    const cookieOptions = { ...getCookieOptions(request), secure: publicUrl.protocol === 'https:' }
