...
export async function GET(request: NextRequest) {
  ...
  const target = request.nextUrl.host
  ...
  if (source === target) return NextResponse.redirect(new URL(returnPath, request.url))

  const state = randomBytes(32).toString('base64url')
  const sourceUrl = new URL('/api/auth/handoff/issue', `${request.nextUrl.protocol}//${source}`)
