...
import { SharedTimelineCanvas, TimelineCanvasEntry } from '@components/Objects/Profile/ProfileTimeline'
...
function TimelineCanvas({ path, selectedUuid, onSelect, onAdd }: { path: RoadmapPathwayDetail; selectedUuid: string | null; onSelect: (uuid: string) => void; onAdd: () => void }) {
  const entries: TimelineCanvasEntry[] = path.blocks.map((item) => ({
    ...
    detail: item.block.lane_category === 'life' ? item.block.lane_category : blockTypeLabels[item.block.block_type],
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-5">
      ...
      />
      <div className="mt-4">
        <Button type="button" variant="outline" className="mt-4" onClick={onAdd}><Plus className="mr-2 h-4 w-4" />Add blank block</Button>
      </div>
    </div>
  )
}
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="grid min-h-[calc(100vh-180px)] grid-cols-1 lg:grid-cols-[320px_minmax(0,1fr)_420px]">
        ...
        <main className="min-w-0 p-5">
          ...
          <TimelineCanvas path={selected} selectedUuid={selectedBlock?.pathway_block_uuid || null} onSelect={(uuid) => { setSelectedBlockUuid(uuid); setPanelMode('detail'); setCanBackToDetail(false) }} onAdd={addBlankBlock} />
        </main>
