...
import { getRoadmapOptions, RoadmapDetail } from '@services/roadmap/roadmap'

type Props = {
  orgslug: string
  initialPathUuid?: string
}
    ...
    return (
      <div className="min-w-[360px] rounded-lg border border-dashed border-gray-200 bg-gray-50 p-6">
        <Select onValueChange={onChange}>
          ...
          <SelectContent>
            {paths.map((item) => <SelectItem key={item.option.roadmap_uuid} value={item.option.roadmap_uuid}>{item.option.end_state_title}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
    )
  ...
  return (
    <section className="min-w-[380px] rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-gray-950">{path.option.end_state_title}</h2>
          <p className="mt-1 text-sm text-gray-500">{path.option.description || 'Pathway details'}</p>
        </div>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        ...
        <div className="rounded-lg bg-gray-50 p-3"><div className="text-xs text-gray-500">Skill fit</div><div className="font-semibold">{path.summary.skill_fit_score ?? '-'}/10</div></div>
        <div className="rounded-lg bg-gray-50 p-3"><div className="text-xs text-gray-500">Lifestyle fit</div><div className="font-semibold">{path.summary.lifestyle_fit_score ?? '-'}/10</div></div>
      </div>
      <div className="mt-5 rounded-lg border border-gray-200 p-4">
        <h3 className="font-semibold text-gray-950">Income range</h3>
        <p className="mt-2 text-sm text-gray-600">{money(path.summary.income_low)} / {money(path.summary.income_mid)} / {money(path.summary.income_high)}</p>
      </div>
      <div className="mt-5">
        <h3 className="font-semibold text-gray-950">Blocks</h3>
        <div className="mt-3 space-y-2">
          {path.events.map((event) => (
            <div key={event.event_uuid} className="rounded-lg border border-gray-100 p-3">
              <div className="text-sm font-semibold text-gray-950">{event.title}</div>
              <div className="mt-1 text-xs text-gray-500">{event.start_date} to {event.is_ongoing ? 'ongoing' : event.end_date || 'open'}</div>
            </div>
          ))}
        </div>
      </div>
...
export default function RoadmapDetailsClient({ orgslug, initialPathUuid }: Props) {
  const session = useLHSession() as any
  const org = useOrg() as any
  const accessToken = session?.data?.tokens?.access_token
  const orgId = org?.id
  const { data: paths = [], isLoading } = useSWR(
    orgId && accessToken ? ['roadmap-detail-paths', orgId, accessToken] : null,
    ([, currentOrgId, token]) => getRoadmapOptions(currentOrgId, token),
    { revalidateOnFocus: false }
  )
  const initial = useMemo(() => initialPathUuid || paths[0]?.option.roadmap_uuid || '', [initialPathUuid, paths])
      ...
      {isLoading ? (
        ...
        <div className="mt-8 flex gap-5 overflow-x-auto pb-6">
          {stackUuids.map((uuid, index) => (
            <DetailStack
              key={`${uuid}-${index}`}
              path={paths.find((path) => path.option.roadmap_uuid === uuid)}
              paths={paths}
          ...
          {!stackUuids.length ? (
            <DetailStack path={undefined} paths={paths} onChange={(nextUuid) => setStackUuids([nextUuid])} onRemove={() => {}} />
          ) : null}
        </div>
      )}
    </GeneralWrapperStyled>
