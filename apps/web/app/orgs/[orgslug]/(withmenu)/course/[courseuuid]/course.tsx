...
const CourseClient = (props: any) => {
  ...
  // Open the chapter containing the next incomplete activity once both course and trailData are ready
  useEffect(() => {
    if (!course?.chapters || hasInitializedChapters.current) return
    if (trailData === undefined) return // wait for trail to load
    ...
    course.chapters.forEach((chapter: any, idx: number) => {
      defaultExpanded[chapter.chapter_uuid] = nextActivityChapterUuid
      ...
      {!course && !org ? (
        ...
        <>
          <GeneralWrapperStyled>
            ...
            <div className="flex flex-col min-[1200px]:flex-row gap-8 pt-2 items-start">
              <div className="w-full min-[1200px]:w-[360px] min-[1200px]:shrink-0 space-y-4">
                {(() => {
                  ...
                  if (showVideo && course.thumbnail_video) {
                    return (
                      <div className="relative inset-0 ring-1 ring-inset ring-black/10 rounded-lg shadow-xl w-full h-[200px] md:max-[1199px]:h-[400px] min-[1200px]:h-[180px] min-[1200px]:w-[320px]">
              ...
              {/* Lessons sidebar */}
              <div className="w-full min-[1200px]:flex-1 mb-10 space-y-3">
                {course.chapters.map((chapter: any, idx: number) => {
                  const isExpanded = expandedChapters[chapter.chapter_uuid] ?? false;
                  return (
                    <div
