...
export default async function PublicResumePage({ params }: { params: Promise<{ orgslug: string; username: string }> }) {
  const { orgslug, username } = await params
  try {
    const org = await getOrganizationContextInfoWithoutCredentials(orgslug, { revalidate: 0 })
    return <PortfolioShell initialShell={await getPublicPortfolio(org.id, username)} orgslug={orgslug} username={username} active="resume" />
  } catch {
    notFound()
  }
}
