...
export default function ProfileResumeClient({
  ...
  const portfolioHref = (card: ResumeFeaturedCard) => {
    ...
    return getUriWithOrg(
      orgslug,
      mode === 'owner'
        ? routePaths.org.profilePortfolioPost(slug)
      ...
      <main className="resume-print-root min-h-screen px-4 py-6 sm:px-6 lg:px-8 print:bg-white print:px-0 print:py-0">
        <div className="resume-no-print mx-auto flex w-full max-w-5xl items-center justify-between gap-3">
          <Button asChild variant="ghost">
            <Link href={getUriWithOrg(orgslug, mode === 'owner' ? routePaths.org.profile() : routePaths.org.user(profileUsername || user.username))}>
