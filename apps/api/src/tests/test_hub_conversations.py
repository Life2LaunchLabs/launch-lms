...
def test_search_creates_private_resumable_rich_conversation(monkeypatch):
    with _session(monkeypatch) as db:
        ...
        assert detail["messages"][0]["intent"] == "search"
        assert detail["messages"][0]["content"] == "career guides"
...
def test_advice_uses_authoritative_history_and_revalidates_resources(monkeypatch):
    with _session(monkeypatch) as db:
        ...
        assert detail["messages"][0]["intent"] == "work"
...
def test_legacy_default_kind_messages_are_not_relabelled_as_explicit_chat(monkeypatch):
    with _session(monkeypatch) as db:
        created = hub_conversations.record_advice(
            db, org_id=7, user_id=11, conversation_uuid=None,
            user_content="Find interview guides", assistant_content="Try these.",
            learner_resource_uuids=[], context_resource_uuids=[], suggested_resource_uuids=[],
            model="test", input_tokens=3, output_tokens=2,
        )
        legacy_user_message = db.exec(select(HubConversationMessage).where(
            HubConversationMessage.message_uuid == created["user_message_uuid"],
        )).one()
        assert legacy_user_message is not None
        legacy_user_message.kind = "chat"
        db.add(legacy_user_message)
        db.commit()
        ...
        assert detail["messages"][0]["intent"] is None
