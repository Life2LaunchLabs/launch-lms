...
def validate_outcomes(outcomes: dict | None, trusted: bool) -> None:
    ...
    if not trusted:
        raise PortfolioActionError(
            "outcomes",
            "authorization",
            "Portfolio outcomes are limited to trusted system badges",
        )
    if outcomes.get("version") != 1 or not isinstance(outcomes.get("actions"), list):
        raise PortfolioActionError(
            "outcomes", "version", "Outcomes must use version 1 and contain actions"
        )
    ids: set[str] = set()
    for action in outcomes["actions"]:
        ...
        if not action_id or action_id in ids:
            raise PortfolioActionError(
                action_id or "outcomes", "id", "Every outcome needs a unique stable id"
            )
        ids.add(action_id)
        if action.get("type") not in ACTION_TYPES:
            raise PortfolioActionError(
                action_id, "type", "Unsupported portfolio outcome"
            )
...
def _mapped(action: dict, context: dict, allowed: set[str]) -> dict:
    ...
    if unknown:
        raise PortfolioActionError(
            action["id"], "fields", f"Unsupported fields: {', '.join(sorted(unknown))}"
        )
...
def _unique_slug(model, portfolio_id: int, title: str, db: Session) -> str:
    ...
    while db.exec(
        select(model).where(model.portfolio_id == portfolio_id, model.slug == candidate)
    ).first():
...
def _portfolio_date(value, field: str) -> str | None:
    ...
    if not re.fullmatch(r"\d{4}-(0[1-9]|1[0-2])(?:-([0-2]\d|3[01]))?", raw):
        raise PortfolioActionError(
            "portfolio-date", field, "Dates must use YYYY-MM or YYYY-MM-DD"
        )
    try:
        datetime.strptime(raw if len(raw) == 10 else f"{raw}-01", "%Y-%m-%d")
    except ValueError as exc:
        raise PortfolioActionError(
            "portfolio-date", field, "Date is not valid"
        ) from exc
    return raw
...
def apply_portfolio_outcomes(
    db: Session,
    user: User,
    activity_run_id: int,
    outcomes: dict,
    context: dict,
    prior_receipts: dict | None = None,
) -> tuple[dict, dict]:
    ...
    for action in outcomes.get("actions", []):
        ...
        if kind == "set_portfolio_fields":
            for key, value in _mapped(action, local, PORTFOLIO_FIELDS).items():
                setattr(portfolio, key, str(value or "").strip())
        elif kind == "enroll_badges":
            badge_uuids = list(
                dict.fromkeys(_resolve(action.get("badge_uuids"), local) or [])
            )[:3]
            enrolled: list[str] = []
            ...
            cover_asset_uuid = str(
                _resolve(action.get("cover_asset_uuid"), local) or ""
            ).strip()
            cover_asset_id = None
            if cover_asset_uuid:
                cover = db.exec(
            ...
            work = WorkItem(
                ...
                source="activity",
                source_reference=f"{run_ref}:{action_id}",
                creation_date=now,
                update_date=now,
                **values,
            )
            ...
            if trait_type not in TRAIT_TYPES:
                raise PortfolioActionError(
                    action_id, "trait_type", "Unsupported trait type"
                )
            ...
            for existing in db.exec(
                select(ProfileTrait).where(
                    ...
                    ProfileTrait.trait_type == trait_type,
                )
            ).all():
                db.delete(existing)
            ...
            if not isinstance(links, list):
                raise PortfolioActionError(action_id, "links", "Links must be a list")
            for index, item in enumerate(links):
                ...
                if parsed.scheme not in {"http", "https"} or not parsed.netloc:
                    raise PortfolioActionError(
                        action_id, "url", "Links must use http or https"
                    )
                db.add(
                    PortfolioLink(
                        link_uuid=f"lnk_{uuid4().hex}",
                        portfolio_id=portfolio.id or 0,
                        link_type=str(item.get("link_type") or "other"),
                        platform=item.get("platform"),
                        label=str(item.get("label") or parsed.netloc),
                        url=url,
            ...
            portfolio.theme_id = str(
                _resolve(action.get("theme_id"), local) or "default"
            )
