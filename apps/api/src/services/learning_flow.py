...
ALLOWED_FACTS = {"has_work", "has_journey", "work_count", "journey_count", "readiness_blockers"}
...
def validate_condition(condition: Any, path: str = "condition") -> None:
    ...
    if not isinstance(left, dict) or left.get("source") not in {"answer", "variable", "fact", "context"}:
...
def evaluate_condition(condition: Any, context: dict, trace: list[dict] | None = None) -> bool:
    if condition in (None, {}):
        result = True
    else:
        op = condition.get("op")
        if op == "and":
            result = all(evaluate_condition(item, context, trace) for item in condition.get("conditions", []))
        elif op == "or":
            result = any(evaluate_condition(item, context, trace) for item in condition.get("conditions", []))
        elif op == "not":
            result = not evaluate_condition(condition.get("condition"), context, trace)
        else:
            left, right = _value(condition.get("left"), context), _value(condition.get("right"), context)
            if op == "exists": result = left is not None
            elif left is None: result = False
            elif op == "eq": result = left == right
            elif op == "ne": result = left != right
            elif op == "in": result = isinstance(right, (list, tuple, set)) and left in right
            elif op == "contains": result = isinstance(left, (list, tuple, set, str)) and right in left
            else:
                try:
                    if op == "gt": result = left > right
                    elif op == "gte": result = left >= right
                    elif op == "lt": result = left < right
                    elif op == "lte": result = left <= right
                    else: result = False
                except TypeError:
                    result = False
...
def validate_flow(flow: dict | None, page_uuids: set[str], required_page_uuids: set[str]) -> list[str]:
    if not flow:
        return []
    ...
    for node_id, node in by_id.items():
        if node.get("type") == "complete": terminals.add(node_id)
        elif node.get("type") == "page" and node.get("page_uuid") in page_uuids: page_nodes.add(str(node.get("page_uuid")))
        else: raise FlowValidationError(f"Flow node {node_id} is invalid")
    if not terminals:
    ...
    def visit(node_id: str) -> None:
        if node_id in stack: raise FlowValidationError("Flow must be acyclic")
        if node_id in seen: return
        stack.add(node_id)
        for edge in outgoing[node_id]: visit(edge["to"])
        stack.remove(node_id); seen.add(node_id)
    visit(entry)
    unreachable = sorted(set(by_id) - seen)
    if unreachable: raise FlowValidationError(f"Unreachable flow nodes: {', '.join(unreachable)}")
    if not required_page_uuids.issubset(page_nodes):
        missing = sorted(required_page_uuids - page_nodes)
        raise FlowValidationError(f"Required pages missing from flow: {', '.join(missing)}")
    for node_id in seen - terminals:
        if not outgoing[node_id]: raise FlowValidationError(f"Flow node {node_id} cannot reach completion")
        priorities = [edge["priority"] for edge in outgoing[node_id]]
        if len(priorities) != len(set(priorities)):
            raise FlowValidationError(f"Flow node {node_id} has ambiguous edge priorities")
    return unreachable
...
def resolve_flow(flow: dict, context: dict) -> ResolvedFlow:
    ...
    while node_id:
        node = nodes[node_id]; node_ids.append(node_id)
        if node.get("type") == "complete": return ResolvedFlow(node_ids, pages, True, trace)
        pages.append(node["page_uuid"])
        candidates = sorted(outgoing.get(node_id, []), key=lambda item: int(item.get("priority", 0)), reverse=True)
        chosen = next((edge for edge in candidates if evaluate_condition(edge.get("condition"), context, trace)), None)
        if not chosen: return ResolvedFlow(node_ids, pages, False, trace)
        node_id = chosen["to"]
