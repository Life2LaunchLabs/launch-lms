...
def _get_activity(db_session: Session, activity_uuid: str) -> LearningActivity:
    activity = db_session.exec(
        select(LearningActivity).where(
            ...
            == _clean_uuid(activity_uuid, "learning_activity_")
        )
    ).first()
    if not activity:
...
def _get_page(db_session: Session, page_uuid: str) -> LearningPage:
    page = db_session.exec(
        select(LearningPage).where(
            LearningPage.page_uuid == _clean_uuid(page_uuid, "learning_page_")
        )
    ).first()
    if not page:
...
def _get_run(
    ...
    statement = select(LearningRun).where(
        LearningRun.run_uuid == _clean_uuid(run_uuid, "learning_run_")
    )
    if actor:
...
def _target_value_type(db_session: Session, org_id: int | None, target: str) -> str:
    if target in _SAFE_IMAGE_VARIABLE_TARGETS:
        return "image"
    if (
        target in _SAFE_CORE_VARIABLE_TARGETS
        or target in _SAFE_PORTFOLIO_VARIABLE_TARGETS
...
def _is_safe_variable_target(
    ...
    for prefix in _SAFE_VARIABLE_PREFIXES:
        if target.startswith(prefix):
            ...
            segments = tail.split(".")
            if any(
                segment in _BLOCKED_VARIABLE_SEGMENTS or segment.startswith("_")
...
def _apply_learning_variables_to_user(
    ...
    for variable in variables:
        ...
        if target == "user.first_name":
            ...
            setattr(
                portfolio,
                _SAFE_PORTFOLIO_VARIABLE_TARGETS[target],
                str(value or "").strip(),
            )
...
def _grade_text_block(page: LearningPage, question: dict, answer: dict) -> dict:
    ...
    inputs = _validate_text_answer(
        content, completion, _normalize_text_answer(content, answer or {})
    )
    scoring = _block_scoring(page, question)
    ...
    points = (
        ...
        if per_input_points
        else _as_float(scoring.get("points"), 1.0)
    )
...
def _grade_answer(
    ...
    score = (
        ...
        else sum(_as_float(value, 0.0) for value in scores if value is not None)
    )
    grading_status = (
        "pending"
        if pending
...
def _has_pending_required_manual_grades(db_session: Session, run: LearningRun) -> bool:
    ...
    manual_pages = [
        ...
        if any(
            ...
            and _block_scoring(page, question).get("mode") == "manual"
            for question in _question_blocks(page)
        )
    ]
    if not manual_pages:
...
def _merge_onboarding_variable_bindings(page: LearningPage, defaults: dict) -> None:
    ...
    if (
        question
        and isinstance(question.get("completion"), dict)
        and question.get("completion")
    ):
...
def _delete_legacy_launch_ready_activities(db_session: Session, badge_id: int) -> None:
    ...
    for activity in legacy_activities:
        ...
        activity_runs = db_session.exec(
            select(LearningActivityRun).where(
                LearningActivityRun.activity_id == activity.id
            )
        ).all()
        for page in pages:
            ...
            for attempt in db_session.exec(
                select(LearningResponseAttempt).where(
                    LearningResponseAttempt.page_id == page.id
                )
            ).all():
...
def _ensure_launch_ready_activity(
    ...
    *,
    ...
    if key == "traits":
        ...
        values_page_uuid, values_block_id = (
            "learning_page_system_onboarding_values",
            "blk_launch_values",
        )
        values = (
            ...
            ("Purpose & Impact", "Justice"),
            ("Purpose & Impact", "Leadership"),
            ("Purpose & Impact", "Nature"),
            ("Purpose & Impact", "Service"),
            ("Purpose & Impact", "Sustainability"),
            ("Purpose & Impact", "Wisdom"),
        )
        pages.insert(
            1,
            {
                "page_uuid": values_page_uuid,
                "block_id": values_block_id,
                "title": "What matters most to you?",
                "kind": "categorized_multi_select",
                "content": {
                    "label": "Values are what matter most to you. They guide your decisions, influence your priorities, and help you choose the kind of life you want to build.",
                    "options": [
                        {"id": label.lower(), "text": label, "category": category}
                        for category, label in values
                    ],
                },
                "completion": {"min_selections": 2, "max_selections": 5},
            },
        )
        outcomes.append(
            {
                ...
                "values": {
                    "$source": "answer",
                    "path": f"{values_page_uuid}.answer.questions.{values_block_id}.option_ids",
                    "default": [],
                },
            }
        )
    activity = db_session.exec(
        select(LearningActivity).where(LearningActivity.activity_uuid == activity_uuid)
    ).first()
    settings = {
        "system_required": True,
        "estimated_minutes": 3,
        "capability": key,
        "outcomes": {"version": 1, "actions": outcomes},
    }
    if flow:
        settings["flow"] = flow
    if not activity:
        activity = LearningActivity(
            ...
            thumbnail_image=LAUNCH_READY_DEFAULT_IMAGES[key],
            icon="sparkles",
            order=order,
            required=True,
            published=True,
        ...
        activity.path_id, activity.badge_id, activity.org_id = (
            path.id or 0,
            badge.id or 0,
            org_id,
        )
    ...
    for page_order, spec in enumerate(pages, start=1):
        page = db_session.exec(
            select(LearningPage).where(LearningPage.page_uuid == spec["page_uuid"])
        ).first()
        content = {
            ...
            "blocks": deepcopy(
                ...
                or [
                    text_block(
                        heading_node(spec["title"]),
                        block_id=f"{spec['block_id']}_heading",
                    )
                ]
            ),
        }
        if spec.get("kind") and not spec.get("blocks"):
            content["blocks"].append(
                question_block(
                    ...
                    completion=spec.get("completion") or {},
                )
            )
        elif not spec.get("blocks"):
            content["blocks"].append(
                text_block(
                    paragraph_node(spec.get("body") or "Continue when you're ready."),
                    block_id=f"{spec['block_id']}_body",
                )
            )
        ...
        if not page:
            page = LearningPage(
                ...
                content=content,
                page_uuid=spec["page_uuid"],
                creation_date=now,
                update_date=now,
            )
        else:
            page.activity_id, page.badge_id, page.org_id, page.order, page.required = (
                activity.id or 0,
                badge.id or 0,
                org_id,
                page_order,
            ...
            page.page_type, page.title, page.content = (
                LearningPageType.STANDARD,
                spec["title"],
                content,
            )
    ...
    stale_system_pages = db_session.exec(
        select(LearningPage).where(LearningPage.activity_id == activity.id)
    ).all()
    for stale_page in stale_system_pages:
        if (
            stale_page.page_uuid.startswith("learning_page_system_onboarding_")
            and stale_page.page_uuid not in desired_page_uuids
        ):
    ...
    for activity_run in db_session.exec(
        ...
        definition.update(
            {"version": 1, "flow": flow, "outcomes": settings["outcomes"]}
        )
        ...
        if was_completed and pages:
            ...
            final_page = db_session.exec(
                select(LearningPage).where(LearningPage.page_uuid == final_page_uuid)
            ).first()
            if final_page:
                final_progress = db_session.exec(
                    select(LearningPageProgress).where(
                ...
                if final_progress:
                    ...
                    final_progress.data = {
                        **(final_progress.data or {}),
                        "invalidated_by_definition_update": True,
                    }
...
def ensure_onboarding_learning_badge(
    ...
    collection = db_session.exec(
        select(BadgeCollection).where(
            BadgeCollection.collection_uuid == ONBOARDING_COLLECTION_UUID
        )
    ).first()
    if not collection:
    ...
    activity = db_session.exec(
        select(LearningActivity).where(
            LearningActivity.activity_uuid == ONBOARDING_ACTIVITY_UUID
        )
    ).first()
    if not activity:
        activity = LearningActivity(
        ...
        activity.thumbnail_image = (
            activity.thumbnail_image or LAUNCH_READY_DEFAULT_IMAGES["identity"]
        )
    ...
    name_page = db_session.exec(
        select(LearningPage).where(LearningPage.page_uuid == ONBOARDING_NAME_PAGE_UUID)
    ).first()
    if not name_page:
        name_page = LearningPage(
            ...
            content={
                ...
                "blocks": [
                    text_block(
                        paragraph_node("Tell us your first and last name."),
                        block_id="blk_onboarding_name_intro",
                    ),
                    question_block(
                        "text_input",
                        {
    ...
    goal_page = db_session.exec(
        select(LearningPage).where(LearningPage.page_uuid == ONBOARDING_GOAL_PAGE_UUID)
    ).first()
    if not goal_page:
        goal_page = LearningPage(
            ...
            content={
                ...
                "blocks": [
                    text_block(
                        heading_node("What are you building toward?"),
                        block_id="blk_onboarding_goal_heading",
                    ),
                    text_block(
                        paragraph_node(
                            "This stays private. We use it to personalize your Launch Ready path and examples."
                        ),
                        block_id="blk_onboarding_goal_private",
                    ),
                    question_block(
                        "multiple_choice",
    ...
    def answer(
        page_uuid: str, block_id: str, suffix: str, default=None, transform=None
    ) -> dict:
        value = {
            "$source": "answer",
            "path": f"{page_uuid}.answer.questions.{block_id}.{suffix}",
        }
    ...
    profile_photo_page, profile_photo_block = (
        "learning_page_system_onboarding_profile_photo",
        "blk_launch_profile_photo",
    )
    profile_details_page, profile_details_block = (
        "learning_page_system_onboarding_profile_details",
        "blk_launch_profile_details",
    )
    ...
    stale_profile_activities = db_session.exec(
        select(LearningActivity).where(
            ...
            func.lower(LearningActivity.title).in_(
                ("complete your profile", "complete your portfolio")
            ),  # type: ignore
            LearningActivity.activity_uuid.notin_(
                set(LAUNCH_READY_ACTIVITY_UUIDS.values())
            ),  # type: ignore
        )
    ).all()
    ...
    _ensure_launch_ready_activity(
        ...
        description="Build an introduction that feels like you and preview the real result.",
        pages=[
            {
                "page_uuid": profile_photo_page,
                "block_id": profile_photo_block,
                "title": "Choose how you show up",
                "kind": "image_upload",
                "content": {
                    "label": "Add a profile photo if you want. You can skip this, and you can change it later."
                },
                "completion": {
                    "required": False,
                    "variable_bindings": {
                        "image": {"target": "user.avatar_image", "value_type": "image"}
                    },
                },
            },
            {
                "page_uuid": profile_details_page,
                "block_id": profile_details_block,
                    ...
                    "inputs": [
                        {
                            "id": "display_name",
                            "label": "Display name",
                            "placeholder": "How you want people to know you",
                            "variant": "single_line",
                            "height": 48,
                        },
                        {
                            "id": "tagline",
                            "label": "Tagline",
                            "placeholder": "Student, maker, and community builder",
                            "variant": "single_line",
                            "height": 48,
                        },
                        {
                ...
                "completion": {
                    "inputs": {
                        "display_name": {"required": True, "min_words": 1},
                        "bio": {"required": True, "min_words": 2},
                        "tagline": {"required": True, "min_words": 1},
                        "location": {"required": False},
                    },
                    "question_mode": "variable",
                    "variable_bindings": {
                        "inputs": {
                            "display_name": {"target": "user.portfolio.display_name"},
                            "tagline": {"target": "user.portfolio.headline"},
                            "bio": {"target": "user.portfolio.short_bio"},
                            "location": {"target": "user.portfolio.location_label"},
                        }
                    },
                },
            },
            {
                "page_uuid": profile_review_page,
                "block_id": "blk_launch_profile_review",
                "title": "This is how you’ll show up",
                "action_label": "Add this to my portfolio",
                "blocks": [
                    text_block(
                        heading_node("This is how you’ll show up"),
                        block_id="blk_profile_review_heading",
                    ),
                    text_block(
                        paragraph_node(
                            "This preview uses the same identity treatment as your portfolio."
                        ),
                        block_id="blk_profile_review_intro",
                    ),
                    {
                        "id": "blk_profile_review_header",
                        "type": "portfolio_preview",
                        "design": {"width": 100},
                        "content": {
                            "variant": "identity_header",
                            "bindings": {
                                "display_name": {
                                    "source": "answer",
                                    "path": f"{profile_details_page}.answer.questions.{profile_details_block}.inputs.display_name.text",
                                    "fallback": "Your name",
                                },
                                "headline": {
                                    "source": "answer",
                                    "path": f"{profile_details_page}.answer.questions.{profile_details_block}.inputs.tagline.text",
                                    "fallback": "Your tagline",
                                },
                                "short_bio": {
                                    "source": "answer",
                                    "path": f"{profile_details_page}.answer.questions.{profile_details_block}.inputs.bio.text",
                                    "fallback": "Your introduction",
                                },
                                "location_label": {
                                    "source": "answer",
                                    "path": f"{profile_details_page}.answer.questions.{profile_details_block}.inputs.location.text",
                                    "fallback": "",
                                },
                                "avatar_url": {
                                    "source": "answer",
                                    "path": f"{profile_photo_page}.answer.questions.{profile_photo_block}.url",
                                    "fallback_binding": {
                                        "source": "variable",
                    ...
                    {
                        "id": "blk_profile_review_details",
                        "type": "button",
                        "design": {
                            "width": 48,
                            "variant": "secondary",
                            "group": "profile_review_actions",
                        },
                        "content": {
                            "label": "Change my introduction",
                            "destination_page_uuid": profile_details_page,
                        },
                    },
                    {
                        "id": "blk_profile_review_photo",
                        "type": "button",
                        "design": {
                            "width": 48,
                            "variant": "secondary",
                        ...
                        "content": {
                            ...
                            "destination_page_uuid": profile_photo_page,
                        },
                    },
                ],
            },
        ...
        outcomes=[
            {
                "id": "set-profile-details",
                "type": "set_portfolio_fields",
                "fields": {
                    "display_name": answer(
                        profile_details_page,
                        profile_details_block,
                        "inputs.display_name.text",
                    ),
                    "short_bio": answer(
                        profile_details_page, profile_details_block, "inputs.bio.text"
                    ),
                    "headline": answer(
                        profile_details_page,
                        profile_details_block,
    ...
    timeline_pages = [
        {
            ...
            "kind": "multiple_choice",
            "content": {
                "label": "Choose what fits best — there’s no wrong kind of experience.",
                "options": [
                    {"id": "education", "text": "School or college"},
                    {"id": "employment", "text": "Job or internship"},
                    {"id": "training", "text": "Program or training"},
                    {"id": "volunteering", "text": "Volunteering or community"},
                    {"id": "experience", "text": "Building something"},
                    {"id": "other", "text": "Exploring what’s next"},
                ],
            },
        ...
        },
        {
                ...
                "inputs": [
                    {
                        ...
                        "adaptive": {
                            ...
                            "values": {
                                ...
                                "employment": {"placeholder": "My first internship"},
                                "training": {"placeholder": "Learning a new skill"},
                                "volunteering": {
                                    "placeholder": "Showing up for my community"
                                },
                                "experience": {
                                    "placeholder": "Building something that matters"
                                },
                        ...
                        "adaptive": {
                            ...
                            "values": {
                                ...
                                "volunteering": {
                                    ...
                                    "placeholder": "Where you volunteer",
                                },
                                "experience": {
                                    "label": "Project, group, or place",
                                    "placeholder": "Where this is happening",
                                },
                                "other": {
                                    "label": "Place or community (optional)",
                                    "placeholder": "Anywhere that matters",
                                },
                            },
                        },
                    },
                    ...
                    },
                    {
                        "id": "summary",
                        "section_id": "summary",
                        "label": "What are you learning, doing, or excited about?",
                        "placeholder": "I’m learning about plants and environmental science.",
                        "variant": "short_answer",
                        "height": 150,
                    },
                ]
            ...
            "completion": {
                "inputs": {
                    "title": {"required": True, "min_words": 1},
                    "organization": {"required": False},
                    "location": {"required": False},
                    "start_date": {"required": False},
                    "summary": {"required": True, "min_words": 2},
                }
            },
        },
        {
    ...
    project_pages = [
        {
            ...
            "content": {
                "inputs": [
                    ...
                    },
                    {
                        "id": "story",
                        "label": "Story",
                        "placeholder": "What did you set out to do? What did you contribute? What changed, or what did you learn?",
                        "variant": "short_answer",
                        "height": 240,
                    },
                ]
            ...
            "completion": {
                "inputs": {
                    ...
                    "story": {"required": True, "min_words": 3},
                }
            },
        },
        {
    ...
    trait_page, trait_block = (
        "learning_page_system_onboarding_traits",
        "blk_launch_traits",
    )
    ...
    _ensure_launch_ready_activity(
        ...
        description="Choose the strengths that feel true now—you can change them as you grow.",
        pages=[
            {
                "page_uuid": trait_page,
                "block_id": trait_block,
                "title": "What feels true about you right now?",
                "kind": "multiple_choice",
                "content": {
                    "label": "Pick two or three qualities you want people to understand about you.",
                    "options": [
                        {"id": item, "text": item.title()} for item in trait_options
                    ],
                },
                "completion": {"min_selections": 2, "max_selections": 3},
            },
            {
                "page_uuid": trait_review_page,
                "block_id": "blk_launch_traits_review",
                "title": "This is what you bring",
                "action_label": "Add what makes me, me",
                "blocks": [
                    text_block(
                        heading_node("This is what you bring"),
                        block_id="blk_traits_review_heading",
                    ),
                    text_block(
                        paragraph_node(
                            "Choose what feels true now. This can change as you do."
                        ),
                        block_id="blk_traits_review_intro",
                    ),
                    {
                        "id": "blk_traits_review_panel",
                        "type": "portfolio_preview",
                        "design": {"width": 100},
                        "content": {
                            "variant": "traits_panel",
                            "bindings": {
                                "traits": {
                                    "source": "answer",
                                    "path": f"{trait_page}.answer.questions.{trait_block}.option_ids",
                                    "fallback": "Curious, Creative",
                                }
                            },
                        },
                    },
                    {
                        "id": "blk_traits_review_change",
                        "type": "button",
                        "design": {"width": 100, "variant": "secondary"},
                        "content": {
                            "label": "Change my choices",
                            "destination_page_uuid": trait_page,
                        },
                    },
                ],
            },
        ...
        outcomes=[
            {
                ...
                "type": "set_traits",
                "trait_type": "strength",
                "values": answer(trait_page, trait_block, "option_ids", []),
            }
        ],
    )

    links_platform_page, links_platform_block = (
        "learning_page_system_onboarding_links_platform",
        "blk_launch_links_platform",
    )
    links_page, links_block = (
        "learning_page_system_onboarding_links",
        "blk_launch_links",
    )
    ...
    _ensure_launch_ready_activity(
        ...
        key="links",
        order=6,
        title="Connect the places you show up",
        description="Connect a public profile or site—or confidently say not yet.",
        pages=[
            {
                "page_uuid": links_platform_page,
                "block_id": links_platform_block,
                "title": "Where do you show up online?",
                "kind": "multiple_choice",
                "content": {
                    "label": "You do not need a personal website.",
                    "options": [
                        {"id": key, "text": label}
                        for key, label in (
                            ...
                            ("tiktok", "TikTok"),
                            ("other", "Another link"),
                            ("not_yet", "I don’t have anything to add yet"),
                        )
                    ],
                },
                "completion": {"min_selections": 1, "max_selections": 1},
            ...
            {
                "page_uuid": links_page,
                "block_id": links_block,
                "title": "Make the destination clear",
                "kind": "text_input",
                "content": {
                    "inputs": [
                        {
                ...
                "completion": {
                    "inputs": {"label": {"required": False}, "url": {"required": False}}
                },
            },
            {
                "page_uuid": links_review_page,
                "block_id": "blk_launch_links_review",
                "title": "Here’s what people can explore",
                "action_label": "Connect this to my portfolio",
                "blocks": [
                    text_block(
                        heading_node("Here’s what people can explore"),
                        block_id="blk_links_review_heading",
                    ),
                    text_block(
                        paragraph_node(
                            "This label and URL will be public. Contact information stays separate."
                        ),
                        block_id="blk_links_review_intro",
                    ),
                    {
                        "id": "blk_links_review_strip",
                        "type": "portfolio_preview",
                        "design": {"width": 100},
                        "content": {
                            "variant": "links_strip",
                            "bindings": {
                                "label": {
                                    "source": "answer",
                                    "path": f"{links_page}.answer.questions.{links_block}.inputs.label.text",
                                    "fallback": "My link",
                                },
                                "url": {
                                    "source": "answer",
                                    "path": f"{links_page}.answer.questions.{links_block}.inputs.url.text",
                                    "fallback": "No link added yet",
                                },
                            },
                        },
                    },
                    {
                        "id": "blk_links_review_change",
                        "type": "button",
                        "design": {"width": 100, "variant": "secondary"},
                        "content": {
                            "label": "Change this link",
                            "destination_page_uuid": links_page,
                        },
                    },
                ],
            },
                ...
                "links": [
                    {
                        ...
                        "platform": answer(
                            links_platform_page,
                            links_platform_block,
                            "option_ids",
                            "other",
                            "first",
                        ),
                        "label": answer(
                            links_page, links_block, "inputs.label.text", "My link"
                        ),
                        "url": answer(links_page, links_block, "inputs.url.text"),
                    }
                ],
            }
        ],
    ...
    badges_pick_page, badges_pick_block = (
        "learning_page_system_onboarding_badges_pick",
        "blk_launch_badges_pick",
    )
    _ensure_launch_ready_activity(
        db_session,
        path=path,
        ...
        pages=[
            {
                "page_uuid": "learning_page_system_onboarding_badges_intro",
                "block_id": "blk_launch_badges_intro",
                "title": "Badges make growth visible",
                "blocks": [
                    text_block(
                        heading_node("Badges make growth visible"),
                        block_id="blk_launch_badges_intro_heading",
                    ),
                    text_block(
                        paragraph_node(
                            "A badge is evidence of something you can do—not just a completion mark. Each path helps you practice, create evidence, and make your progress easier to share."
                        ),
                        block_id="blk_launch_badges_intro_body",
                    ),
                    text_block(
                        paragraph_node(
            ...
            {
                "page_uuid": badges_pick_page,
                "block_id": badges_pick_block,
                "title": "Pick your starting badges",
                "action_label": "Start these badges",
                "kind": "multiple_choice",
                "content": {
                    "label": "Choose up to three. You can always start more later.",
                    "options_binding": {
                        "source": "variable",
                        "path": "learning.available_badge_options",
                    },
                },
        ...
        outcomes=[
            {
                "id": "enroll-starting-badges",
                "type": "enroll_badges",
                "badge_uuids": answer(
                    badges_pick_page, badges_pick_block, "option_ids", []
                ),
            }
        ],
    )
    ...
    launch_page, launch_block = (
        "learning_page_system_onboarding_launch",
        "blk_launch_confirm",
    )
    ...
    _ensure_launch_ready_activity(
        ...
        order=8,
        title="You did it!",
        description="Celebrate finishing Launch Ready, publish your portfolio, and get ready to share it.",
        pages=[
            {
                "page_uuid": launch_done_page,
                "block_id": "blk_launch_done",
                "title": "You did it!",
                "blocks": [
                    text_block(
                        heading_node("You did it!"), block_id="blk_launch_done_heading"
                    ),
                    text_block(
                        paragraph_node(
                    ...
                    text_block(
                        paragraph_node(
                            "Finishing Launch Ready also unlocks your resume view, formatted for printing and sharing with employers."
                        ),
                        block_id="blk_launch_done_resume",
                    ),
                ],
            },
            {
                "page_uuid": launch_page,
                "block_id": launch_block,
                "title": "Set your portfolio public",
                "action_label": "Publish my portfolio",
                "kind": "multiple_choice",
                "content": {
                    "label": "Turn on your public portfolio so people with the link can view it.",
                    "options": [
                        {"id": "set_public", "text": "Make my portfolio public"}
                    ],
                },
                "completion": {"min_selections": 1, "max_selections": 1},
            },
            {
                "page_uuid": launch_share_page,
                "block_id": "blk_launch_share",
                "title": "Share your next step",
                "action_label": "Finish Launch Ready",
                "blocks": [
                    text_block(
                        heading_node("Share your next step"),
                        block_id="blk_launch_share_heading",
                    ),
                    text_block(
                        paragraph_node(
                            "Your public portfolio and print-friendly resume are ready from your Portfolio page. Copy your link when you want to send it to a mentor, teacher, employer, or someone who should see what you can do."
                        ),
                        block_id="blk_launch_share_body",
                    ),
                    {
                        "id": "blk_launch_share_panel",
                        "type": "portfolio_preview",
                        "design": {"width": 100},
                        "content": {
                            "variant": "share_panel",
                    ...
                    text_block(
                        paragraph_node(
                            "You can keep editing after this. Publishing is a starting line, not a final draft."
                        ),
                        block_id="blk_launch_share_note",
                    ),
                ],
            },
        ],
        outcomes=[
            {"id": "confirm-public", "type": "confirm_privacy"},
            {"id": "publish-public-portfolio", "type": "publish_portfolio"},
        ],
    )
    ...
    if "settings" in patch:
        ...
        try:
            ...
            validate_outcomes(
                ...
                bool(badge and _is_system_object(badge)),
            )
        except (FlowValidationError, PortfolioActionError) as exc:
            raise HTTPException(status_code=422, detail=str(exc)) from exc
    for key, value in patch.items():
...
async def convert_page_variants_to_flow(
    ...
    page = next(
        (
            item
            for item in pages
            if item.page_uuid == _clean_uuid(page_uuid, "learning_page_")
        ),
        None,
    )
...
def _validate_variable_key(key: str) -> str:
    ...
    if not normalized or any(
        ...
        or segment in _BLOCKED_VARIABLE_SEGMENTS
        for segment in segments
    ):
        raise HTTPException(
            status_code=422,
...
def _get_variable(db_session: Session, variable_uuid: str) -> LearningVariable:
    variable = db_session.exec(
        select(LearningVariable).where(
            ...
            == _clean_uuid(variable_uuid, "learning_variable_")
        )
    ).first()
    if not variable:
...
async def complete_page(
    ...
    if (
        ...
        can_complete, completion_result = _activity_meets_completion_rules(
...
async def grade_learning_response(
    ...
    max_score = (
        ...
        or sum(
            _question_block_points(page, question)
            for question in _question_blocks(page)
        )
...
async def confer_award(
    ...
    if issuing_org_id is not None:
        ...
        if not _get_approved_issuer_authorization(
            db_session, badge.id or 0, issuing_org_id
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def get_award(request: Request, award_uuid: str, db_session: Session) -> dict:
    award = db_session.exec(
        select(LearningBadgeAward).where(
            LearningBadgeAward.award_uuid == _clean_uuid(award_uuid, "award_")
        )
    ).first()
    if not award:
