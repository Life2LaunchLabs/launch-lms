...
if [ -n "$LAUNCHLMS_SQL_CONNECTION_STRING" ]; then
    # Extract host and port from postgresql://user:pass@host:port/db
    DB_HOST=$(echo "$LAUNCHLMS_SQL_CONNECTION_STRING" | sed -n 's/.*@\([^:]*\):\([0-9]*\)\/.*/\1/p')
    DB_PORT=$(echo "$LAUNCHLMS_SQL_CONNECTION_STRING" | sed -n 's/.*@\([^:]*\):\([0-9]*\)\/.*/\2/p')
...
if [ -n "$LAUNCHLMS_REDIS_CONNECTION_STRING" ]; then
    # Extract host and port from redis://host:port/db or redis://host:port
    REDIS_HOST=$(echo "$LAUNCHLMS_REDIS_CONNECTION_STRING" | sed -n 's|redis://\([^:/]*\):\([0-9]*\).*|\1|p')
    REDIS_PORT=$(echo "$LAUNCHLMS_REDIS_CONNECTION_STRING" | sed -n 's|redis://\([^:/]*\):\([0-9]*\).*|\2|p')
    ...
    if [ -z "$REDIS_HOST" ]; then
        ...
        REDIS_HOST=$(echo "$LAUNCHLMS_REDIS_CONNECTION_STRING" | sed -n 's|redis://\([^:/]*\).*|\1|p')
    fi
...
PORT=${LAUNCHLMS_PORT:-9000}
HOST=${HOSTNAME:-0.0.0.0}

echo "Starting Launch LMS backend on ${HOST}:${PORT}..."
