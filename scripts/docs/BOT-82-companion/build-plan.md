...
Phase 3 actions use a true split button. Activating the main **Work on this plan** area revalidates the destination, navigates there and grants Hub a visible, narrowly scoped edit run. The arrow segment opens alternatives such as **Open plan without editing**; it never activates the primary action. While granted, the companion header presents a visibly active **Focused session** with its plain-language goal, a disclosure showing the immutable granted scope, an editable goal, and an explicit **End** control. Goal edits are durable session events and contextual model input, not learner-authored chat bubbles. A changed goal is allowed when established in conversation, but the run always has one current explicit goal. A request to act on another plan or surface must first resolve unfinished objects and conclude the current run, then use a new learner-granted split action; incidental out-of-scope opportunities may be offered as a next session only after the current goal is complete.

For an explicit request to create a plan, the pre-grant exchange stays deliberately short: Hub asks whether to go create it and presents the split action. It does not draft the plan structure in chat before authorization; discovery, questions and proposals happen progressively inside the focused session. Learner-facing advisor replies must remain below the replayable message limit, and provider output, persisted history and client request history each enforce that boundary independently. Transport validation details are translated into a short recoverable interface message rather than exposed as raw API JSON.

Entering, ending and reopening focused mode each leave a quiet hairline boundary in transcript order. An ended boundary becomes a **Restore session** control on hover; restoring revalidates and reopens the same scope, returns to its native route, preserves its activity history, and asks one short orienting question before doing more work. A later learner request that clearly belongs to that ended scope may also receive a prominent reopen-session action from Hub. Ordinary destinations are not decision controls: render supporting page references as quiet link chips (and, as message rendering evolves, inline with the referenced phrase), visually distinct from split grant and restore actions.
...
Editing progresses through semantic events rather than a polling watcher. Granting scope, saving or cancelling an object, answering a question and resolving a review each become contextual turns in the same conversation, but interface-generated context is rendered as a quiet session receipt rather than a learner chat bubble. Object decisions include the final visible values and whether each Hub proposal was accepted, customized or rejected. Multi-object operations wait until their entire review batch is resolved before automatically resuming Hub; intermediate saves remain visible as aggregating session receipts and the learner may still send an ordinary question. The orchestrator then continues until its next explicit boundary: another supported edit, a clear question awaiting the learner, or a proposed conclusion with an in-transcript **Finish session** action. Closing the companion may hide the response but does not suppress continuation caused by a completed review batch. A bare acknowledgement is not a valid stopping state.
